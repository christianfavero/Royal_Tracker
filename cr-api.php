...
class ClashRoyaleAPI {
    ...
    public function getPlayerCards(string $gamertag): array {
        ...
        if (isset($profile['error']) || !isset($profile['cards'])) 
            return $profile;
        $cards = $profile['cards'];
        // Calcola i livelli e le evoluzioni per le carte prima di fare altro
    ...
    public function getYouTubeVideosCached(string $query, int $maxResults = 6): array {
        ...
        // Se la cache è scaduta o non esiste, chiama YouTube
        ...
        // Salva il risultato nel file se la risposta è valida
        if (isset($data['items']))
